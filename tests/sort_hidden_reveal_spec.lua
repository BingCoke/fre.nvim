...
describe("fre ticket 07 sort hidden and reveal", function()
  ...
  it("reveals nested snapshot paths without invoking window primitives", function()
    ...
    assert.are.equal("a/b/target.txt", instance._pending_reveal.relative)
  end)
  ...
  it("keeps only the newest hidden pending target and applies it on open", function()
    local instance = ready({ ["a.txt"] = "a", ["b.txt"] = "b", ["c.txt"] = "c" })
    instance:reveal("a.txt")
    instance:reveal("c.txt")
    assert.are.equal("c.txt", instance._pending_reveal.relative)
    instance:open()

    assert.is_nil(instance._pending_reveal)
    assert.are.same(instance:get_pos("c.txt"), vim.api.nvim_win_get_cursor(0))
  end)

  it("selects a deterministic existing window without opening another", function()
    local instance = ready({ ["a.txt"] = "a", ["b.txt"] = "b", ["c.txt"] = "c" })
    instance:open()
    ...
    local chosen = math.min(first, second)
    local other = chosen == first and second or first

    instance:reveal("c.txt")
    assert.are.same(before, vim.fn.win_findbuf(instance.bufnr))
    assert.are.same(instance:get_pos("c.txt"), vim.api.nvim_win_get_cursor(chosen))
    assert.are.same(instance:get_pos("a.txt"), vim.api.nvim_win_get_cursor(other))
  end)

  it("keeps cross-tab targets pending until the instance opens in the current tab", function()
    local instance = ready({ ["a.txt"] = "a", ["b.txt"] = "b", ["c.txt"] = "c" })
    instance:open()
